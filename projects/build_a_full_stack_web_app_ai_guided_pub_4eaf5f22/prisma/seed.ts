...
export async function main(options: { allowProductionBootstrap?: boolean } = {}) {
  ...
  try {
    ...
    const citizenIdFieldsV1Raw = [
      ...
      { id: "old_id_number", type: "text", label: "Số CCCD/CMND đã được cấp", required: false, visibleWhen: { field: "issue_reason", operator: "in", value: ["cap_doi", "cap_lai"] } },
      { id: "permanent_address", type: "text", label: "Nơi thường trú", required: true },
      { id: "phone_number", type: "text", label: "Số điện thoại", required: true },
    ...
    const citizenIdRulesV1Raw = [
      ...
      { id: "cc_v1_cond_old_id", type: "conditional_required", fieldId: "old_id_number", params: { condition: { field: "issue_reason", operator: "in", value: ["cap_doi", "cap_lai"] } }, message: "Vui lòng nhập số CCCD/CMND đã được cấp.", suggestion: "Cần thiết khi cấp đổi hoặc cấp lại.", severity: "error", orderNumber: 6 },
